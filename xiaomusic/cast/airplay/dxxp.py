...
_log = logging.getLogger("xiaomusic.cast.airplay.dxxp")
    ...
    if(len(chunk) > 0):
        ...
        def get_next_frame(_in, buffer, rec=0):
            ...
            try:
                ...
                if leng > 0:
                    # start 0, end 4
                    # 上游此处用 print, 但未知 DAAP 标签是常态
                    # (每次 SET_PARAMETER 解析元数据都会遇到), 会刷屏 stdout,
                    # 改为 debug 日志。
                    try:
                        _tag = _in[0:4].decode("utf-8", errors="replace")
                    except Exception:
                        _tag = repr(_in[0:4])
                    _log.debug("未注册的 DAAP 标签: %s; length: %s", _tag, leng)
                pass
            finally:
                pass
