...
from xiaomusic.cast.const import (
    TRANSPORT_STATE_PAUSED,
    TRANSPORT_STATE_PLAYING,
    TRANSPORT_STATE_STOPPED,
    TRANSPORT_STATE_TRANSITIONING,
)
from xiaomusic.cast.dlna.eventing import EventManager, build_last_change_event, clamp_timeout
from xiaomusic.cast.dlna.media_buffer import MediaBuffer
from xiaomusic.cast.dlna.cover import (
...
from xiaomusic.cast.dlna.soap_handler import (
    handle_soap_request,
    parse_soap_action,
    parse_soap_body,
)
from xiaomusic.cast.dlna.templates import (
    AVTRANSPORT_SCPD,
...
class DeviceServer:
    ...
    def _check_ffmpeg(self) -> str | None:
        """检测 ffmpeg（优先 xiaomusic 配置，其次项目目录，最后系统 PATH）"""
        if self._ffmpeg_checked:
            return self._ffmpeg_path
        self._ffmpeg_checked = True
        # 1) xiaomusic 配置的 ffmpeg_location（默认 ./ffmpeg/bin，非 Docker 安装用）
        configured = getattr(self.config, "ffmpeg_location", "") if self.config else ""
        if configured:
            for name in ("ffmpeg.exe", "ffmpeg"):
                candidate = os.path.join(configured, name)
                if os.path.isfile(candidate):
                    self._ffmpeg_path = candidate
                    log.info(f"ffmpeg 可用 (配置 ffmpeg_location): {self._ffmpeg_path}")
                    return self._ffmpeg_path
            # 配置项也可能直接指向可执行文件本身
            if os.path.isfile(configured):
                self._ffmpeg_path = configured
                log.info(f"ffmpeg 可用 (配置 ffmpeg_location): {self._ffmpeg_path}")
                return self._ffmpeg_path
    ...
    async def _ffmpeg_seek(
        ...
        in_fd, in_path = tempfile.mkstemp(suffix=suffix, prefix="xiaomusic_si_")
        out_fd, out_path = tempfile.mkstemp(suffix=suffix, prefix="xiaomusic_so_")
        proc = None
        try:
    ...
    async def _handle_media_proxy(self, request: web.Request) -> web.StreamResponse:
        ...
        # 播完自动断开：登记这条推流通道。
        # 注册放在所有校验之后，避免上面的提前 return 泄漏登记项。
        proxy_task = asyncio.current_task()
        stream_handle = stream_guard.open(
            f"dlna:{udn or 'unknown'}",
            lambda: proxy_task.cancel() if proxy_task and not proxy_task.done() else None,
        )
        ...
        try:
            ...
            if bytes_sent >= content_length:
                return response
            log.info(f"收到断开指令，中止媒体代理推流 ({bytes_sent}/{content_length} bytes)")
            raise
